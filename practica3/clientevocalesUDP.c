...
//	servinfo=NULL; // como ya no tenemos la memoria, dejamos de apuntarla para evitar acceder a ella por error
	...
	while ((len = read(0, msg, MAX_BUFF_SIZE)) > 0) { 
		...
		if ((sentbytes=sendto(sock,msg ,len ,0, servinfo->ai_addr, servinfo->ai_addrlen)) < 0) { //envía datos al socket                                                           //MODIFICADO
	...
	if (sendto(sock,&fin ,sizeof fin ,0, servinfo->ai_addr, servinfo->ai_addrlen) < 0) {                                          //MODIFICADO
		perror("Error de escritura en el socket");
		exit(1);
	}
	if (f_verbose) {
	...
	//recibe del servidor el número de vocales recibidas:
																													...
																													//MODIFICADO
	if (recvfrom(sock,&num ,sizeof num,0,servinfo->ai_addr, &servinfo->ai_addrlen) < 0) {                                                        //COMPLETAR
...
//	if (f_verbose) { printf("Estableciendo la comunicación a través del socket (connect)... "); fflush(stdout); }
