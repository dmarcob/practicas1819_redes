...
int main(int argc, char * argv[]) {
	...
	int sock; //conn; // descriptores de socket                                                               //MODIFICADO
...
//		printsockaddr(&caddr);
		...
		num = 0;
		do {
		  if ((readbytes = recvfrom(sock,msg, BUFF_SIZE,0,(struct sockaddr *)&caddr,&clen)) < 0) {                                //MODIFICADO
		...
		if (sendto(sock, &netNum,sizeof netNum,0,(struct sockaddr *)&caddr,clen) < 0) {                                                                         //MODIFICADO
...
//	if (f_verbose) { printf("Permitiendo conexiones entrantes (listen)... "); fflush(stdout); }
