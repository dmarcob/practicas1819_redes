...
/*struct rcftp_msg construir(char *vector,int leido) { //longEfect = bytes leidos en teclado
	...
	sendbuffer.sum=0;
	strcpy(sendbuffer.buffer,vector);
	sendbuffer.sum=xsum(&sendbuffer,sizeof(sendbuffer));
	return sendbuffer;
}*/
/*void construirMensajeRCFTP(struct rcftp_msg* sendbuffer, char *vector,int leido) { //longEfect = bytes leidos en teclado
	// empezar sin flags activos
	sendbuffer->flags=F_NOFLAGS;
	// construir el mensaje válido ***********************************
	sendbuffer->version= RCFTP_VERSION_1;
